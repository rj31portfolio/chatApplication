                ...
                if ($conn->connect_error) {
                    ...
                    if (!$conn->select_db($dbName)) {
                        if (!$conn->query("CREATE DATABASE `$dbName`")) {
                            ...
                            if (!$conn->select_db($dbName)) {
                                $errors[] = 'Database was created but could not be selected.';
                            }
                        }
                    }
                    $conn->close();
                    ...
                    if (empty($errors)) {
                        // Create config file
                        $configPath = __DIR__ . '/includes/config-custom.php';
                        $configContent = <<<PHP
<?php
/**
 * Custom configuration settings for the AI Chatbot System
 */
...
PHP;
        ...
        case 3:
            ...
            if (empty($errors)) {
                ...
                // Load default config and database functions
                ...
                // Create admin user
                $result = createUser($adminUsername, $adminPassword, $adminEmail, 'super_admin');
                
                if ($result !== false) {
                    header('Location: install.php?step=4');
                    exit;
                } else {
                    $errors[] = 'Failed to create admin user. Please check database connection.';
                }
            }
            break;
            
        case 4:
            // Final step - create example data
            $createExamples = isset($_POST['create_examples']) && $_POST['create_examples'] == '1';
            
            // Load configurations and database functions
            $configPath = __DIR__ . '/includes/config-custom.php';
            if (file_exists($configPath)) {
                require_once $configPath;
            }
            ...
            if ($createExamples) {
                // Create example businesses and responses
                createExampleData();
            }
            
            // Create a .installed file to prevent running the installer again
...
function createExampleData() {
    ...
    $adminId = createUser('business_admin', 'password123', 'admin@example.com', 'admin');
...
<body>
    <div class="container">
            ...
            <?php elseif ($step == 2): ?>
                ...
                <form method="post" action="install.php?step=2">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">If the database does not exist, the installer will try to create it.</div>
                    </div>
            ...
            <?php elseif ($step == 3): ?>
                ...
                <form method="post" action="install.php?step=3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" class="form-control" id="admin_email" name="admin_email" value="<?php echo htmlspecialchars($_POST['admin_email'] ?? ''); ?>" required>
                    </div>
        ...
        <div class="text-center mt-4">
            <p class="text-muted small">AI Chatbot System &copy; <?php echo date('Y'); ?></p>
        </div>
    </div>
